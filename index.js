...
app.get("/", ifNotLoggedin, (req, res, next) => {
  dbConnection
    .execute("SELECT `name`,`class` FROM `users` WHERE `id`=?", [
      req.session.userID,
    ])
    .then(([rows]) => {
      res.render("home", {
    ...
    if (validation_result.isEmpty()) {
      ...
      bcrypt
        ...
        .then((hash_pass) => {
          ...
          dbConnection
            .execute(
              "INSERT INTO `users`(`name`,`email`,`class`,`password`) VALUES(?,?,?,?)",
              [user_name, user_email, user_class, hash_pass]
            )
            .then((result) => {
              res.send(
...
app.post(
  ...
  [
    body("user_email", "Invalid email address!")
      ...
      .custom((value) => {
        return dbConnection
          .execute("SELECT `email` FROM `enquiry` WHERE `email`=?", [value])
          .then(([rows]) => {
            if (rows.length > 0) {
              return Promise.reject("This E-mail already in use!");
            }
    ...
    body("user_phr", "The number must be of minimum length 10 characters")
      .trim()
      .isLength({ min: 10 }),
    body("user_pho", "The number must be of minimum length 10 characters")
      .trim()
      .isLength({ min: 10 }),
  ...
  (req, res, next) => {
    ...
    if (validation_result.isEmpty()) {
      ...
      dbConnection
        .execute(
          "INSERT INTO `enquiry`( `name`,`email`,`user_date`,`f_name`,`m_name`,`f_occ`,`m_occ`,`user_radd`,`user_oadd`, `user_phr`, `user_pho`, `user_lca`, `user_lcn`,`user_rel`,`user_na`,`class`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
          [
            user_name,
            user_email,
            user_date,
